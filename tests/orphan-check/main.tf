...
locals {
  ...
  # fails the check on exactly the accounts that have nothing to report, so
  # coalesce before the `for`.
  #
  ...
  # would become two.
  orphan_endpoints = [
    for e in coalesce(data.temporalcloud_nexus_endpoints.all.nexus_endpoints, []) : "nexus-endpoint:${e.name}"
    if startswith(e.name, var.test_resource_prefix)
  ]

  orphan_namespaces = [
    for n in coalesce(data.temporalcloud_namespaces.all.namespaces, []) : "namespace:${n.name}"
